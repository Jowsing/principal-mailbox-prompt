...
1. Decide mode:
   ...
   - Materialize prompt/steps/files/checklist into numbered files:
     `node .codex/skills/principal-mailbox-prompt/scripts/task-pack.mjs --mode write --root . --out .codex/principal-mailbox-task-pack`
...
3. Preserve immutable contract exactly:
   - Flexible: framework, routing, styling, folder layout, UI implementation.
...
- Generated prompt/code states that only APIs/globals/payloads/jumps are fixed; stack/project structure is flexible.
- Generated prompt/code keeps business logic detailed and leaves style to later design/custom prompt input.
